...
import (
	...
	"github.com/tobiassjosten/nogfx/pkg/procs/achaea/procs"
...
func TestLearnMultipleLessons(t *testing.T) {
	...
	tcs := map[string]tst.IOTestCase{
		"learn 20": {
			...
			Inoutputs: []pkg.Inoutput{
				...
				tst.IOOut("Y continues your training in X.").RemoveOutput(0),
				tst.IO(
					"learn 15 x from y",
					"15 of 35 lessons learned, 0 seconds remaining.",
				),

				tst.IOOut("Y begins the lesson in X.").RemoveOutput(0),
				tst.IOOut("Y continues your training in X.").RemoveOutput(0),
				tst.IO(
					"learn 5 x from y",
	...
	for name, tc := range tcs {
		t.Run(name, func(t *testing.T) {
			mod := procs.NewLearnMultipleLessons()
			tc.Eval(t, mod)
		})
	}
}
